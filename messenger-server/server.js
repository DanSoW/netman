...
io.on("connection", (socket) => {
    ...
    socket.on("authentication", async (data) => {
        ...
        socket.emit("authentication_success", "Авторизация прошла успешно!");
        ...
        socket.on("get_new_messages", async () => {
            // Делегирование функции поиска новых сообщений сервису
            let chats = await messages.allUserChats(user_data.users_id, user_data.access_token);

            // Отправка новых сообщений пользователю, который запросил выполнение данного запроса
        ...
        socket.on("send_private_message", async (data) => {
            ...
            const value = data;
            // Поиск текущего индекса подключения
            const indexCurrent = duExistsValueIndex(dataUsers, socket.id);
            // Поиск пользователя, которому отправляют сообщение, в списке текущих подключений
            ...
            if ((indexCurrent >= 0) && (!currentUser.rooms_id)) {
                ...
                // Отправка события о создании нового чата
            ...
            // Отправка сообщения в определённую группу подключений
