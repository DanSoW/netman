...
export const useAuth = () => {
    ...
    const login = useCallback((typeAuth, jwtAccessToken, jwtRefreshToken, usersId, atrib, mod) => {
        ...
        localStorage.setItem(AuthStoragesConstants.main, JSON.stringify({
            users_id: usersId,
            type_auth: typeAuth,
    ...
    const logout = useCallback(async () => {
        ...
        localStorage.removeItem(AuthStoragesConstants.main);
    }, []);

    useEffect(() => {
        const data = JSON.parse(localStorage.getItem(AuthStoragesConstants.main));
